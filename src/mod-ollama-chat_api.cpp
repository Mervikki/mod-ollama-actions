...
static std::string StripThinkSections(const std::string& response)
{
    if (response.empty())
        return response;

    static std::string const openTag = "<think>";
    static std::string const closeTag = "</think>";
    ...
    {
        size_t openPos = response.find(openTag, cursor);
        size_t closePos = response.find(closeTag, cursor);

        if (openPos == std::string::npos && closePos == std::string::npos)
        {
            cleaned.append(response, cursor, std::string::npos);
            break;
        }
        ...
        {
            cleaned.append(response, cursor, closePos - cursor);
            cursor = closePos + closeTag.size();
            continue;
        }

        cleaned.append(response, cursor, openPos - cursor);

        size_t endPos = response.find(closeTag, openPos + openTag.size());
        if (endPos == std::string::npos)
            break;

        cursor = endPos + closeTag.size();
    }
...
std::string QueryOllamaAPI(const std::string& prompt, const std::string& systemOverride)
{
    ...
    bool hadThinkTags = botReply.find("<think>") != std::string::npos ||
        botReply.find("</think>") != std::string::npos;

    botReply = ExtractTextBetweenDoubleQuotes(botReply);
    botReply = StripThinkSections(botReply);

    static std::string const openTag = "<think>";
    static std::string const closeTag = "</think>";
    for (size_t pos = botReply.find(openTag); pos != std::string::npos;
         pos = botReply.find(openTag))
        botReply.erase(pos, openTag.size());

    for (size_t pos = botReply.find(closeTag); pos != std::string::npos;
         pos = botReply.find(closeTag))
        botReply.erase(pos, closeTag.size());

    if (hadThinkTags && g_DebugEnabled)
    {
        LOG_INFO("server.loading", "[OllamaChat] Debug: Think tags were present and stripped from output.");
    }
