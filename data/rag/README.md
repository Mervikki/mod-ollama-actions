...
## Generate RAG Data From AzerothCore DB (WotLK)

This module includes a scaffold exporter script that pulls factual data from your local
`acore_world` database and writes RAG JSON files in this folder.

Script path:
- `/home/cachyd/acore/modules/mod-ollama-chat/apps/rag_export_wotlk.py`

What it exports:
- `wow_wotlk_quests_db.json`
- `wow_wotlk_items_db.json`
- `wow_wotlk_creatures_db.json`
- `wow_wotlk_gameobjects_db.json`

The script reads `WorldDatabaseInfo` from `worldserver.conf` by default and uses
the local `mysql` CLI (no Python DB dependency required).

### Example

```bash
python3 /home/cachyd/acore/modules/mod-ollama-chat/apps/rag_export_wotlk.py \
  --worldserver-conf /home/cachyd/azeroth-server/etc/worldserver.conf \
  --output-dir /home/cachyd/acore/modules/mod-ollama-chat/data/rag \
  --limit 800
```
...
```bash
python3 /home/cachyd/acore/modules/mod-ollama-chat/apps/rag_export_wotlk.py --dry-run
```

### Notes

- Generated entries are tagged with `wotlk` and patch tag `3.3.5a` by default.
- You can tune entry volume using `--limit`.
- Existing handcrafted JSON files are not modified unless filenames overlap.
